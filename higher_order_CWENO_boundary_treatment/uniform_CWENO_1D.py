...
from higher_order_CWENO_boundary_treatment.utils.numerical_helpers import (
...
class Uniform_CWENO_1D:
    ...
    Gamma = [
        np.array(
            [
                [1, 0, 0, 0],  # coefficents tables for
                [0, 2, 0, 0],  # reconstructions according to
                [-1 / 4, -3, 3, 0],  # Cravero et. al (2017)
                [1, 7, -12, 4],
            ]
        ),
        np.array([[1, 0, 0, 0], [2, 2, 0, 0], [-1 / 4, 3, 3, 0], [0, -5, 0, 4]]),
        np.array(
            [
                [1, 0, 0, 0, 0],
                [4, 2, 0, 0, 0],
    ...
    def compute_reconstruction_polynomial(self):
        ...
        temp_coeffs = [
            np.multiply(nonlin_weights[:, i : i + 1], polynomials[i])
            for i in range(len(polynomials))
        ]
    ...
    def compute_interpolants(self):
        ...
        coeffs.append(
            np.dot(
                self.diffs[: -self.G, :],  # optimal polynomial
                self.Gamma[self.g][: self.G + 1, : self.G + 1],
            )
        )
        coeffs.append(
            np.dot(
                self.diffs[: -self.G, : self.g + 1],  # P1 with same offset as optimal
                self.Gamma[self.g][: self.g + 1, : self.g + 1],
            )
        )
        incrementer = 1
        for idx in range(self.g - 1, -1, -1):  # remaining polynomials
            coeffs.append(
                np.dot(
                    self.diffs[incrementer : -self.G + incrementer, : self.g + 1],
    ...
    def nonlinear_weights(self, d, indicators):
        ...
        alphas = d / np.apply_along_axis(
            lambda x: np.power(x, self.p), 1, indicators + self.eps
        )
    ...
    def reconstruct_cell_interfaces(self):
        ...
        if self.params is None:
            ...
            # reconstruction polynomials at borders are centered around corresponding
            # inner neighbor cell
                ...
                for i in range(j, g + 1):
                    if (i + j) % 2 == 0:
                        factorials = (
                            factorial(j)
                            * factorial(i)
                            / (factorial(j - l) * factorial(i - l))
                        )
                        powerTerms = 2 ** (2 * l + 1 - j - i - int(i == j)) / (
                            j + i - 2 * l + 1
                        )
...
class CWENO3_1D(Uniform_CWENO_1D):
    ...
    def compute_reconstruction_polynomial(self):
        ...
        nonlin_weights = self.nonlinear_weights(d, indicators)
        temp_coeffs = [
            np.multiply(nonlin_weights[:, i : i + 1], self.polynomials[i])
        ...
        if self.params is None:
            ...
            rec_coeffs[self.g : -self.g, :] = temp_coeffs[0] / np.power(
                self.h, range(self.G + 1)
            )
...
class CWENO5_1D(Uniform_CWENO_1D):
    ...
    def compute_indicators(self):
        ...
        indicators[:, 0] = (
            1680 * np.power(self.polynomials[0][:, 1], 2)
            ...
            + 1051404 * np.power(self.polynomials[0][:, 4], 2)
        ) / 1680
        for i in range(1, len(self.polynomials)):
            indicators[:, i] = np.power(
                self.polynomials[i][:, 1], 2
            ) + 13 / 3 * np.power(self.polynomials[i][:, 2], 2)
    ...
    def inner_boundary(self, side, p_opt):
        ...
        omegas = alphas / np.sum(alphas)
        rec_coeffs = (
            p_0 * omegas[0] + p_1 * omegas[1] + p_2 * omegas[2] + p_3 * omegas[3]
        )
...
class CWENO7_1D(Uniform_CWENO_1D):
    ...
    def compute_indicators(self):
        ...
        indicators = np.zeros([self.diffs.shape[0] - self.G, len(self.polynomials)])
        indicators[:, 0] = (
            887040 * np.power(self.polynomials[0][:, 1], 2)
            ...
            + 3725568
            * np.multiply(self.polynomials[0][:, 2], self.polynomials[0][:, 4])
            + 1378080
            * np.multiply(self.polynomials[0][:, 2], self.polynomials[0][:, 6])
            + 34694352 * np.power(self.polynomials[0][:, 3], 2)
            + 55942920
            * np.multiply(self.polynomials[0][:, 3], self.polynomials[0][:, 5])
            + 555141312 * np.power(self.polynomials[0][:, 4], 2)
            + 1342648560
            * np.multiply(self.polynomials[0][:, 4], self.polynomials[0][:, 6])
            + 13878542425 * np.power(self.polynomials[0][:, 5], 2)
            + 499627530135 * np.power(self.polynomials[0][:, 6], 2)
        ) / 887040
        for i in range(1, len(self.polynomials)):
            indicators[:, i] = (
                240 * np.power(self.polynomials[i][:, 1], 2)
                + 120
                * np.multiply(self.polynomials[i][:, 1], self.polynomials[i][:, 3])
                + 1040 * np.power(self.polynomials[i][:, 2], 2)
                + 9387 * np.power(self.polynomials[i][:, 3], 2)
            ) / 240
        return indicators
    ...
    def outer_boundary(self, side, p_opt):
        ...
        if side == "left":
            IS[0] = (
                887040 * p_0[1] ** 2
                - 10644480 * p_0[1] * p_0[2]
                + 48343680 * p_0[1] * p_0[3]
                - 196922880 * p_0[1] * p_0[4]
                + 758530080 * p_0[1] * p_0[5]
                - 2828105280 * p_0[1] * p_0[6]
                + 35777280 * p_0[2] ** 2
                - 359251200 * p_0[2] * p_0[3]
                + 1600397568 * p_0[2] * p_0[4]
                - 6682737600 * p_0[2] * p_0[5]
                + 26813492640 * p_0[2] * p_0[6]
                + 1004672592 * p_0[3] ** 2
                - 9967224960 * p_0[3] * p_0[4]
                + 46144878120 * p_0[3] * p_0[5]
                - 204086116080 * p_0[3] * p_0[6]
                + 27882182592 * p_0[4] ** 2
                - 292452098400 * p_0[4] * p_0[5]
                + 1462297528560 * p_0[4] * p_0[6]
                + 881856884425 * p_0[5] ** 2
                - 10190971222740 * p_0[5] * p_0[6]
                + 34480494630315 * p_0[6] ** 2
            ) / 887040
            IS[2] = (
                240 * p_2[1] ** 2
                - 2880 * p_2[1] * p_2[2]
    ...
    def middle_boundary(self, side, p_opt):
        ...
        if side == "left":
            IS[0] = (
                887040 * p_0[1] ** 2
                - 7096320 * p_0[1] * p_0[2]
                + 21732480 * p_0[1] * p_0[3]
                - 60318720 * p_0[1] * p_0[4]
                + 159778080 * p_0[1] * p_0[5]
                - 412917120 * p_0[1] * p_0[6]
                + 18036480 * p_0[2] ** 2
                - 133056000 * p_0[2] * p_0[3]
                + 429504768 * p_0[2] * p_0[4]
                - 1291382400 * p_0[2] * p_0[5]
                + 3721623840 * p_0[2] * p_0[6]
                + 306128592 * p_0[3] ** 2
                - 2422506240 * p_0[3] * p_0[4]
                + 8697930120 * p_0[3] * p_0[5]
                - 29233401120 * p_0[3] * p_0[6]
                + 6100915392 * p_0[4] ** 2
                - 55104033600 * p_0[4] * p_0[5]
                + 226615326960 * p_0[4] * p_0[6]
                + 161827574425 * p_0[5] ** 2
                - 1703570936760 * p_0[5] * p_0[6]
                + 5894755440615 * p_0[6] ** 2
            ) / 887040
            IS[2] = (
                240 * p_2[1] ** 2
                - 1920 * p_2[1] * p_2[2]
    ...
    def inner_boundary(self, side, p_opt):
        ...
        if side == "left":
            IS[0] = (
                ...
                + 5765760 * p_0[1] * p_0[3]
                - 8870400 * p_0[1] * p_0[4]
                + 13416480 * p_0[1] * p_0[5]
                - 20180160 * p_0[1] * p_0[6]
                + 7392000 * p_0[2] ** 2
                - 34594560 * p_0[2] * p_0[3]
                + 67592448 * p_0[2] * p_0[4]
                - 122337600 * p_0[2] * p_0[5]
                + 212937120 * p_0[2] * p_0[6]
                + 78602832 * p_0[3] ** 2
                - 455495040 * p_0[3] * p_0[4]
                + 1078810920 * p_0[3] * p_0[5]
                - 2304363600 * p_0[3] * p_0[6]
                + 1292271552 * p_0[4] ** 2
                - 9223552800 * p_0[4] * p_0[5]
                + 26073323760 * p_0[4] * p_0[6]
                + 32401508425 * p_0[5] ** 2
                - 277013485980 * p_0[5] * p_0[6]
                + 1166705407755 * p_0[6] ** 2
            ) / 887040
            IS[2] = (
                240 * p_2[1] ** 2
                - 960 * p_2[1] * p_2[2]
